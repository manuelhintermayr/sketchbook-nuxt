...
export class OutlineEffect
{
	...
	constructor(world: World)
	{
		...
		const boundResize = (): void => this.onResize();
		window.addEventListener('resize', boundResize);
		this.world.disposers.push(() => window.removeEventListener('resize', boundResize));

		// Free our render-target + materials when the World tears down.
		this.world.disposers.push(() =>
		{
			try { this.depthRT.dispose(); } catch (_e) { /* noop */ }
			try { this.outlineMat.dispose(); } catch (_e) { /* noop */ }
			try { this.depthMat.dispose(); } catch (_e) { /* noop */ }
		});
	}
