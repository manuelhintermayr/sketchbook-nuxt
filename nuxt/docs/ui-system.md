...
The UI rebuild that came with the Nuxt port replaced every `document.createElement`-driven overlay from the parent edition with a Vue 3 SFC. Tokens carry over verbatim. This doc is the catalogue.
...
Vue port of the lil-gui control surface from the parent edition. `useEngineParams()` exposes the engine's `params` object as a reactive proxy; each control binds to a specific key with `v-model`. The same `gui.controllersRecursive()` cache `SettingsModal` uses also feeds DebugPanel — so changes from either surface fire the original `onChange` handlers.
