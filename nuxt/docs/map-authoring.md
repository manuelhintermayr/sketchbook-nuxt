...
The bundled level (`build/assets/world.glb`, served from the parent repo via Vite's `@fs` allow-list) is authored in Blender and exported with userData passed through. The four sandbox scenes (`engine/world/sandboxes/Test*.ts`) demonstrate the same conventions in code and are good copy-paste starting points.

This doc mirrors the parent edition's map-authoring guide — the engine is unchanged, only the file paths differ.
...
GLB files (`world.glb`, `car.glb`, `airplane.glb`, …) are not duplicated into `nuxt/`. They live in the parent repo's `build/assets/`, and Vite's `nuxt.config.ts` has an `@fs` allow-list entry that lets the dev server + the production build resolve them. From browser code, paths are still `/build/assets/world.glb` (relative to the dev server root) — same as the Webpack edition.

If you need to add a new GLB asset:

1. Drop it in `../build/assets/` (parent repo's existing path).
2. Reference it as `/build/assets/your-file.glb` from engine code.
3. Optional: pre-load it through `LoadingManager.loadGLTF(path)` so it's waited on before the world reveals.

If the new asset is a Vue-side asset (icon, image inside an overlay), put it in `nuxt/public/` and reference as `/your-asset.png`.
