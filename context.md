...
Assets are vendored locally (no `@fs` allow-list, no parent-repo dependency) — every GLB / image / audio file the engine needs lives under `public/`.
...
| Branches | work on `feature/nuxt-upgrade`; never push to `master` |
...
Same per-frame loop as the upstream edition (the engine is unchanged):
...
3. **Do not** rename or move existing public API (`Sketchbook.World`, scene class exports) without checking call sites in `EngineHost.vue`.
4. **Do not** vendor copyrighted assets without checking license. The Inthenew port replaced six hotlinked images with DALL-E equivalents specifically because of this — see the [upstream sketchbook-upgraded README](https://github.com/manuelhintermayr/sketchbook-upgraded#august-2025---version-060---daynight-cycle-boats--rocketship) v0.6.0 section.
...
- The [upstream `manuelhintermayr/sketchbook-upgraded`](https://github.com/manuelhintermayr/sketchbook-upgraded) repo carries the original Webpack edition's `README.md`, `CHANGELOG.md`, and `docs/` — reach for them when you need to know **why** an engine choice exists.
